...
const PhotoCards = () => {
  ...
  return (
      ...
      {albumDataCopy.reverse().map((items, idx)=> {
        ...
        return  <div key={items.albumid} className='CPCards w-90 h-80 bg-[#ececfa] border border-black/20 relative'>
        <div className='w-full h-50 '>
        <img src={items.coverImgObj.src} alt={items.coverImgObj.originalName} className='w-full rounded-t-lg h-full object-cover object-center'/>
        </div>
        ...
        <h2 className='text-black/70 capitalize'>{project?.project}</h2>
        </div>
        ...
        <button className="cursor-pointer hover:bg-black/10 p-1 rounded-full transition-all" onClick={()=> {
          if(isOptionsID === idx) {
            setisOptionsID(null)
          }
          else {
        ...
        {isOptionsID === idx ? <Actions state={{setisOptionsID}} albData = {items} /> : null}

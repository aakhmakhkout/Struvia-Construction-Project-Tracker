        ...
        <div className="flex gap-10 items-center">
          ...
          <div className="w-60 h-15">
            <div className="flex absolute top-0 right-4 flex-col w-60 z-10">
              {userData.map((items) => {
                return (
                  <div key={items.id}>
                    {items.role === role ? (
                      <div
                        className="flex items-center h-15 justify-evenly"
                        onLoad={() => {
                          {
              ...
              <div
                className={`bg-[#f0f5ff] ${showHidden} flex-col gap-2 py-3 rounded-b-xl items-center w-60 NavToggle`}
              >
                {userData.map((items) => {
                  if (items.role !== role) {
                    return (
                      <ul key={items.id}>
                        <li>
                          <button
                            className="font-bold bg-[#7745a7] text-white w-30 py-1 rounded-md cursor-pointer"
                            onClick={() => {

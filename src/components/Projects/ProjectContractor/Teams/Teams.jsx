...
export default function Teams({ state }) {
  ...
  const [Pagination, setPagination] = useState(fallbackObj);
  ...
  return (
    <div className="fixed inset-0 bg-black/30 backdrop-blur-[2px] z-50 flex justify-center items-center">
      <div className="bg-[#f0f5ff] w-[65%] h-[80%] CPCards p-4 border border-black/20 relative flex flex-col justify-between">
        <div>
          <Top
            teamsModalState={state}
            AddtmState={{ isATMformOpen, setisATMformOpen }}
            searchBarState={{ setsearchBarData }}
            tmstOpt={{ settmStatusOptions }}
          />
          <TeamCards
            AddtmState={{ setisATMformOpen }}
            searchData={searchBarData}
            tmstOpt={tmStatusOptions}
            pageIndexes={{ Pagination }}
          />
        </div>
        <Bottom
